    ...
    with open(f'aerest{self.nombre}.{self.paso}', 'w') as f:
      with open(f'vapest{self.nombre}.{self.paso}', 'w') as g:
        for k in range(self.nz1):
          f.write(f'{k} {self.aerm[k]} {self.imax[k]} {self.jmax[k]} {self.max_aer_perturbation[k]}\n')
          g.write(f'{k} {self.vapm[k]} {self.lmax[k]} {self.mmax[k]} {self.max_vap_perturbation[k]}\n')

def pert_aerosol_y_vapor_de_estela():
  # Este programa lee la salida del la nube en 3-D y calcula la perturbacion de aerosoles y vapor en la estela
  dimension = Dimension()
  dimension.aeroana()
  dimension.grabar()
